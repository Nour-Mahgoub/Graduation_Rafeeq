...
class SpeechRecognitionNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('volume_threshold', 0.04)
    ...
    def _audio_loop(self):
        ...
        try:
            while self._running:
                ...
                # Wake word: use energy-based VAD (model can't detect "rafeeq")
                # Robot wakes when voice energy stays above threshold for
                # WAKE_CONFIRM_CHUNKS consecutive chunks (~0.5 s of sustained speech)
                WAKE_CONFIRM_CHUNKS = int(SAMPLE_RATE / CHUNK * 0.5)
                sustained = 0
                ...
                while self._running:
                    ...
                    if rms > self.volume_threshold:
                        sustained += 1
                        if sustained >= WAKE_CONFIRM_CHUNKS:
                            self.get_logger().info(
                                f'Voice detected (RMS={rms:.3f}) — activated!'
                            )
                            break
                    else:
                        sustained = 0  # reset on silence
